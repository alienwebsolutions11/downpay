...
import logo from "./Photos/down.jpg"
import alien from './Photos/alien.jpg'
import {
...
try {
  const themeRes = await fetch(
    `https://${session.shop}/admin/api/2025-07/themes.json`,
    {
      headers: {
  ...
  if (mainTheme) {
    const assetRes = await fetch(
      `https://${session.shop}/admin/api/2025-07/themes/${mainTheme.id}/assets.json?asset[key]=config/settings_data.json`,
      {
        headers: {
          "X-Shopify-Access-Token": session.accessToken,
          "Content-Type": "application/json",
    ...
    logoPath =
      ...
      currentSettings.logo_image ||
      currentSettings.logo_src ||
      currentSettings.brand_logo ||
      null;

    //  Search inside sections
    if (!logoPath && currentSettings.sections) {
      for (const section of Object.values(currentSettings.sections)) {
        const s = section?.settings || {};

        if (s.logo || s.logo_image || s.logo_src) {
          logoPath = s.logo || s.logo_image || s.logo_src;
          break;
        }
      }
    }

    console.log("RAW LOGO PATH:", logoPath);

 

    if (logoPath) {
      // case 1: shopify://shop_images/xxx.png
      if (logoPath.startsWith("shopify://")) {
        const fileName = logoPath.split("/").pop();

        shopLogoUrl = `https://${session.shop}/cdn/shop/files/${fileName}`;
      }

      // case 2: already full CDN URL
      else if (logoPath.includes("cdn.shopify.com")) {
        shopLogoUrl = logoPath;
      }
  ...
  console.log("FINAL LOGO URL:", shopLogoUrl);
...
//for testing i comment this and add a new 
...
console.log("🟢 DB QUERY SUCCESS, rows:", purchases.length); 
...
export const action = async ({ request }) => {
  ...
  if (intent === "delete") {
    ...
    // 1️ Get selling plan group id //comment this for fix cloudflare
    ...
    //   );
    // });
const planRowResult = await db.query(
  `SELECT selling_plan_group_id
   FROM purchase_table
   WHERE id = $1
    AND shop = $2`,
...
const planRow = planRowResult[0];
    ...
    // 4️⃣ Delete from DB // comment for fix cloudflare
    ...
    //   );
    // });
    await db.query("DELETE FROM purchase_table WHERE id = $1", [purchaseId]);
    return json({ deleted: true });

  }

//commnet for cloudflare fixing 
...
const [loadingId, setLoadingId] = useState(null);
  ...
  const { purchases, session_token, session_shop, totalProducts,themeExtensionActive ,shopLogoUrl} = useLoaderData();
console.log("SHOP LOGO URL:", shopLogoUrl);
  ...
  !showSetupCard ? (
    <Card>
      <Box padding="600">
        <BlockStack gap="500" inlineAlign="center">
          ...
          <Button primary onClick={() => setShowSetupCard(true)}>
            Begin Setup
          </Button>
    ...
    />
        ...
        <BlockStack gap="400" inlineAlign="center">
          ...
          <Text>
            You're all set to start creating deposit-based purchase options.
          </Text>
                    ...
                    <Card>
                      <ResourceList
                        ...
                        renderItem={(item) => {
                          ...
                          let detailsText = "";
            let mediaSource = image_url || shopLogoUrl || alien;
...
if (selection_type === "Products" && products) {
  ...
  detailsText = remainingProducts !== null
    ? `Applied to all products (${remainingProducts} available, excluded ${excludedCount})`
