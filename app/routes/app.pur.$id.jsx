...
//comment for solve the cloudflare
...
//comment for above fix coud 
    ...
    //   );
    // });
await db.query("DELETE FROM purchase_table WHERE id = $1", [params.id]);
    return redirect("/app");
  }
...
console.timeEnd("DB_UPDATE");
//comment for above solution cloud
  // const planRow = await new Promise((resolve, reject) => {
  //   db.query(
  ...
  //   );
  // });
const planRowResult2 = await db.query(
  `SELECT selling_plan_group_id, selling_plan_id
   FROM purchase_table
   WHERE id = $1`,
  [params.id]
);
const planRow = planRowResult2[0];
...
// setAllowFullPayment(Number(purchase.payin_full) === 1);
...
useEffect(() => {
  ...
  setDepositType(purchase.deposit_type || "percentage");
  setDepositAmount(purchase.deposit_amount || "");
  ...
  setDays(purchase.remaining_balance_days || "");
  setSelectedDate(
    purchase.remaining_balance_date
      ? new Date(purchase.remaining_balance_date)
      : new Date()
  );

  // ✅ FIX HERE
  setSelected(purchase.deposit_options_display || "always");
